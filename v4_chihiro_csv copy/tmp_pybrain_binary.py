...
def main():
	group_by = 120*1000
	order = 1
	filename = "training_set.129A_readings.cal.csv.matrix_dataset_binary.p"
	f = open(filename, "rb")
	...
	pybrain_dataset = pybrain.datasets.SupervisedDataSet(matrix_dataset.shape[1] - 1, 1)

	for reading in matrix_dataset:
		pybrain_dataset.appendLinked( list(reading[:-1]), [reading[-1]])

	print pybrain_dataset

	net = pybrain.tools.shortcuts.buildNetwork(matrix_dataset.shape[1] - 1, matrix_dataset.shape[1] - 1, 1)
	
	trainer = pybrain.supervised.trainers.BackpropTrainer(net, pybrain_dataset)
